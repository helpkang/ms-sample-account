...
import com.kal.framework.integration.adaptor.ECommerceAdaptor;
 ...
 * @since 2012-03-23
 */
public class ECommerceAdaptorImpl implements ECommerceAdaptor {
	
	private static Logger logger = Logger.getLogger(ECommerceAdaptorImpl.class);
	 private HttpAdaptor httpAdaptor;
	 
	 public ECommerceAdaptorImpl() {
	 ...
	 */
     public void setConnectionTimeout(int timeout){
	        httpAdaptor.setConnectionTimeout(timeout);
	 }
	 ...
	 */
     public void connect() throws Exception	 {
	     httpAdaptor.afterPropertiesSet();
	 }
	...
	public String call(byte[] input){
		String output = null;
		try{
			output = new String(httpAdaptor.send(input));
    ...
    public ArrayList getHeadersByName(String header)
    {
        return httpAdaptor.getHeadersByName(header);
    }

    public void setJSessionId(String jsessionId)
    {
        httpAdaptor.setCookie("JSESSIONID", jsessionId);
    }
}
