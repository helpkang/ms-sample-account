...
import com.koreanair.external.dx.vo.AirCalendarAdvancedSearchRequest;
 ...
 *
 * API tests for AirCalendarApi 
 */
public class AirCalendarApiTest {


    private AirCalendarApi api;
    ...
    public void setup() {
        ...
        api = JAXRSClientFactory.create("https://nodeA1.test.api.amadeus.com/v2", AirCalendarApi.class, providers);
    ...
    /**
     * Advanced search for air calendars (cheapest price per day)
     *
     * Air calendar enables a customer who has flexible dates to see the cheapest price option per day over a range of days. The number of days of flexibility can be sent in input to indicate the number of days before and after the requested date to be searched. For a round trip request, the standard range of days is up to 7, meaning three days before and three days after the requested date. For a one-way request, the range is up to 15 days, meaning seven days before and seven days after the requested date. By default if no travellers are specified, the results are returned for one adult. Also, if no flexibility is specified, for both one way and round-trip the results returned are for a range of seven days, meaning three days before and three days after the requested date. The standard flexibility ranges can be extended using implementation configuration settings. Note that only the price is returned, not the details of the flights. Dictionary structure is available on page [Maps in dictionary documentation](http://e-travel.mkt.amadeus.net/dxapi/display/documentation.html#!/Maps/get). 
     *
     * @throws ApiException
     *          if the Api call fails
     */
    @Test
    public void advancedAirCalendarTest() {
        AirCalendarAdvancedSearchRequest advancedCalendarInputs = null;
        String lang = null;
        //AirCalendarReply response = api.advancedAirCalendar(advancedCalendarInputs, lang);
    ...
    /**
     * Retrieve air calendar information (cheapest price per day)
     *
     * Air calendar enables a customer who has flexible dates to see the cheapest price option per day over a range of days. The number of days of flexibility can be sent in input to indicate the number of days before and after the requested date to be searched. For a round trip request, the standard range of days is up to 7, meaning three days before and three days after the requested date. For a one-way request, the range is up to 15 days, meaning seven days before and seven days after the requested date. By default if no travellers are specified, the results are returned for one adult. Also, if no flexibility is specified, for both one way and round-trip the results returned are for a range of seven days, meaning three days before and three days after the requested date. The standard flexibility ranges can be extended using implementation configuration settings. Note that only the price is returned, not the details of the flights. Dictionary structure is available on page [Maps in dictionary documentation](http://e-travel.mkt.amadeus.net/dxapi/display/documentation.html#!/Maps/get). 
     *
     * @throws ApiException
     ...
     */
    @Test
    public void airCalendarTest() {
        LocalDateTime departureDateTime = null;
        String originLocationCode = null;
        String destinationLocationCode = null;
        List<String> commercialFareFamilies = null;
        LocalDateTime returnDateTime = null;
        Integer flexibility = null;
        Integer maxPrice = null;
        List<String> travelers = null;
        Integer departureTimeWindow = null;
        Integer returnTimeWindow = null;
        String alternateOriginLocationCode = null;
        String alternateDestinationLocationCode = null;
        List<String> corporateCodes = null;
        Boolean directFlights = null;
        String promotionCode = null;
        String promotionAirlineCode = null;
        Boolean showUnavailableEntries = null;
        String lang = null;
        //AirCalendarReply response = api.airCalendar(departureDateTime, originLocationCode, destinationLocationCode, commercialFareFamilies, returnDateTime, flexibility, maxPrice, travelers, departureTimeWindow, returnTimeWindow, alternateOriginLocationCode, alternateDestinationLocationCode, corporateCodes, directFlights, promotionCode, promotionAirlineCode, showUnavailableEntries, lang);
        //assertNotNull(response);
        // TODO: test validations
