...
import com.koreanair.external.dx.vo.AirOffersAdvancedSearchRequest;
...
public interface AirOfferApi  {

    /**
     * Advanced search for air offers with additional services
     *
     * This is a shopping transaction that returns a list of air offers and associated services for a given set of criteria. This entry point supports all the advanced use-cases on top of the simple entry point (i.e. all traveler associations, PTC discounts). 
     *
     */
    @POST
    ...
    @ApiOperation(value = "Advanced search for air offers with additional services", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = AirOffersListReply.class),
    ...
    public AirOffersListReply advancedAirShopping(AirOffersAdvancedSearchRequest advancedInputs, @QueryParam("lang")String lang);

    /**
     * Retrieve air offers with additional services
     *
     * This is a shopping transaction that returns a list of air offers and associated services for a given set of criteria. So far only one way and round trip are supported. Note #1: In case the number of travelers is not specified, by default the Digital API returns the result for one adult. Note #2: Complex Itinerary and Round the World are not directly supported (they will be supported as multi steps transaction). Note #3: Dictionary structure is available on page [Maps in dictionary documentation](http://e-travel.mkt.amadeus.net/dxapi/display/documentation.html#!/Maps/get). 
     *
     */
    @GET
    @Path("/search/air-offers")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Retrieve air offers with additional services", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = AirOffersListReply.class),
        @ApiResponse(code = 400, message = "Bad request", response = Messages.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public AirOffersListReply airShopping(@QueryParam("departureDateTime")LocalDateTime departureDateTime, @QueryParam("originLocationCode")String originLocationCode, @QueryParam("destinationLocationCode")String destinationLocationCode, @QueryParam("commercialFareFamilies")List<String> commercialFareFamilies, @QueryParam("returnDateTime")LocalDateTime returnDateTime, @QueryParam("travelers")List<String> travelers, @QueryParam("max")Integer max, @QueryParam("maxPrice")Integer maxPrice, @QueryParam("departureTimeWindow")Integer departureTimeWindow, @QueryParam("returnTimeWindow")Integer returnTimeWindow, @QueryParam("alternateOriginLocationCode")String alternateOriginLocationCode, @QueryParam("alternateDestinationLocationCode")String alternateDestinationLocationCode, @QueryParam("corporateCodes")List<String> corporateCodes, @QueryParam("directFlights")@DefaultValue("false") Boolean directFlights, @QueryParam("promotionCode")String promotionCode, @QueryParam("promotionAirlineCode")String promotionAirlineCode, @QueryParam("showSoldOut")@DefaultValue("false") Boolean showSoldOut, @QueryParam("lang")String lang);

    /**
     * Retrieve an offer
     *
     * Retrieve an Offer based on its Offer id. Dictionary structure is available on page [Maps in dictionary documentation](http://e-travel.mkt.amadeus.net/dxapi/display/documentation.html#!/Maps/get). 
     *
     */
    @GET
    @Path("/search/air-offers/{airOfferId}")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    ...
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 400, message = "Bad request", response = Messages.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public AirOfferReply retrieveAirOffer(@PathParam("airOfferId") String airOfferId, @QueryParam("lang")String lang);
}
