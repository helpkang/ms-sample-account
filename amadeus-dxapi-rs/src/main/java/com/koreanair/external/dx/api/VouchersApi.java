...
import com.koreanair.external.dx.vo.RetrieveVoucherReply;
import com.koreanair.external.dx.vo.VoucherPostRequest;
import com.koreanair.external.dx.vo.VoucherRetrieveRequest;
import com.koreanair.external.dx.vo.VoucherTopUpRequest;
...
public interface VouchersApi  {
     ...
     *
     * Create a new voucher based on the provided information. If an amount and a paymentMethod are provided, the voucher is created with an initial amount on it.
     *
     */
    @POST
    ...
    @ApiOperation(value = "Create a new voucher", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "'OK, Voucher details returned with the associated voucherId'  code    | title ------- | ------------------------------------- 32399   | PAYMENT FAILED ", response = RetrieveVoucherReply.class),
        @ApiResponse(code = 400, message = "Received input with an unexpected format  code    | title ------- | ------------------------------------- 1       | CHECK FORMAT 4926    | INVALID DATA RECEIVED 3654    | INVALID NUMBER OF DECIMAL POSITIONS 6323    | INVALID CURRENCY CODE 37576   | INVALID PAYMENT METHOD ", response = Messages.class),
        @ApiResponse(code = 500, message = "Unable to process  code    | title ------- | ------------------------------------- 11      | UNABLE TO PROCESS 33175   | VOUCHER CREATION FAILED 39074   | VOUCHER CREATION FAILED - AMOUNT STILL AUTHORIZED "),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public RetrieveVoucherReply createVoucher(VoucherPostRequest postVoucherBody);

    /**
     * Retrieve voucher details
     *
     * Retrieve the details on a voucher given:    - the voucher Identifier    - the associated verification code (if needed)
     *
     */
    @POST
    @Path("/purchase/vouchers/{voucherId}")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Retrieve voucher details", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK, Voucher details returned", response = RetrieveVoucherReply.class),
        @ApiResponse(code = 400, message = "Bad request code    | title ------- | ------------------------------------- 1       | CHECK FORMAT", response = Messages.class),
        @ApiResponse(code = 404, message = "Resource not found code    | title ------- | ------------------------------------- 38828   | VOUCHER NOT FOUND", response = Messages.class),
        @ApiResponse(code = 500, message = "Internal error code    | title ------- | ------------------------------------- 11      | UNABLE TO PROCESS", response = Messages.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public RetrieveVoucherReply retrieveVoucher(@PathParam("voucherId") String voucherId, VoucherRetrieveRequest voucherRetrieveRequest);

    /**
     * Top-up money on a voucher
     *
     * Top-up additional amount on a given voucher. In order to top-up money: - Voucher should already exist and still be in &#39;Usable&#39; status - Amount should be in the same currency 
     *
     */
    @POST
    @Path("/purchase/vouchers/{voucherId}/top-up")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Top-up money on a voucher", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "'OK, Voucher details returned with the associated voucherId'  code    | title ------- | ------------------------------------- 27802   | VOUCHER NOT ACTIVE 32399   | PAYMENT FAILED ", response = RetrieveVoucherReply.class),
        @ApiResponse(code = 400, message = "Received input with an unexpected format  code    | title ------- | ------------------------------------- 1       | CHECK FORMAT 4926    | INVALID DATA RECEIVED 3654    | INVALID NUMBER OF DECIMAL POSITIONS 6323    | INVALID CURRENCY CODE 37576   | INVALID PAYMENT METHOD ", response = Messages.class),
        @ApiResponse(code = 404, message = "Resource not found with the received inputs  code    | title ------- | ------------------------------------- 38828   | VOUCHER NOT FOUND ", response = Messages.class),
        @ApiResponse(code = 500, message = "Unable to process  code    | title ------- | ------------------------------------- 11      | UNABLE TO PROCESS 39075   | VOUCHER UPDATE FAILED 39076   | VOUCHER UPDATE FAILED - AMOUNT STILL AUTHORIZED "),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public RetrieveVoucherReply topUpVoucher(@PathParam("voucherId") String voucherId, VoucherTopUpRequest postVoucherTopUpBody);
}
