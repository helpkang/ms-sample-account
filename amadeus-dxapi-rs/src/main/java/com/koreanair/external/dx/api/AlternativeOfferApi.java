...
import com.koreanair.external.dx.vo.AlternativeOffersReply;
import com.koreanair.external.dx.vo.JourneyAlternativeFlightOffersReply;
...
public interface AlternativeOfferApi  {

    /**
     * Retrieve alternative flight offers for this journey
     *
     * Retrieve possible alternative flights for this journey, depending on the search type passed in input. E.g. Free standby options on an earlier flight, or a paid change to another flight. Offers may change one or more flights from the journey. These offers must then be passed through the Payment flow (like ancillaires or upgrade), whether the offers are free or paid 
     *
     */
    @GET
    @Path("/search/alternative-offers/by-journey")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Retrieve alternative flight offers for this journey", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = JourneyAlternativeFlightOffersReply.class),
    ...
    public JourneyAlternativeFlightOffersReply getAlternativeFlightoffersFromJourney(@QueryParam("journeyId")String journeyId, @QueryParam("searchType")String searchType);

    /**
     * Search and propose alternative offers for a given bound, which has been disrupted.
     *
     * In the event of a disruption(flight cancellation, delay or schedule change), the airline may proactively reaccommodate  the passenger to an alternative flight(aka recommended flight).   If the passenger does not find the recommended flight to be suitable, he can search and select alternative flight(s) for  the disrupted bound free of charge. This alternative flight is returned as part of an &#39;alternative-offer&#39;.   The entry point takes as input the Order identifier (PNR) and the disrupted bound identifier in order to search for and return suitable alternative offers. 
     *
     */
    @GET
    @Path("/search/alternative-offers")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Search and propose alternative offers for a given bound, which has been disrupted.", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = AlternativeOffersReply.class),
        @ApiResponse(code = 400, message = "400 Bad Request", response = Messages.class),
        @ApiResponse(code = 404, message = "404 Not Found / Functional", response = Messages.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public AlternativeOffersReply getAlternativeOffers(@QueryParam("orderId")String orderId, @QueryParam("boundId")String boundId, @QueryParam("lastName")String lastName, @QueryParam("departureDateTime")LocalDateTime departureDateTime, @QueryParam("includeNearbyCities")@DefaultValue("false") Boolean includeNearbyCities);
}
