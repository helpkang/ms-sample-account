...
import com.koreanair.external.dx.vo.FlightInformationReply;
import com.koreanair.external.dx.vo.FlightTimetableListReply;
...
public interface FlightInformationApi  {
     ...
     */
    @GET
    @Path("/search/flight-details")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get Flight Information", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Flight information by flight number", response = FlightInformationReply.class),
    ...
    public FlightInformationReply standaloneFlightInfo(@QueryParam("marketingAirlineCode")String marketingAirlineCode, @QueryParam("marketingFlightNumber")String marketingFlightNumber, @QueryParam("departureDate")LocalDate departureDate);

    /**
     * Get Flight timetables
     *
     * Display existing flights and the day(s) of the week they operate for a given departure and destination and from a 7 days range starting from the date given in input . If no date is specified, the current date is taken. This entry point does not provide seats availability for the returned flights 
     *
     */
    @GET
    @Path("/search/flight-timetables")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get Flight timetables", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "Flight timetable information", response = FlightTimetableListReply.class),
        @ApiResponse(code = 400, message = "Bad request", response = Messages.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public FlightTimetableListReply standaloneFlightTimetable(@QueryParam("originLocationCode")String originLocationCode, @QueryParam("destinationLocationCode")String destinationLocationCode, @QueryParam("departureDate")LocalDate departureDate);
}
