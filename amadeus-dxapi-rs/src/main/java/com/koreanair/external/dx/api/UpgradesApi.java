...
import com.koreanair.external.dx.vo.JourneyUpgradePriceReply;
import com.koreanair.external.dx.vo.JourneyUpgradePriceRequest;
import com.koreanair.external.dx.vo.JourneyUpgradeShopReply;
...
public interface UpgradesApi  {

    /**
     * Compute total price of selected upgrade options for this journey
     *
     * This operation will calculate the total price of the chosen combination (not necessarily a simple sum, depending on the pricing and discount rules) for the upgrades passed. The purchase transaction for upgrades can then be performed using the standard journey payment call at /checkin/journeys/{journeyId}/payment-records 
     *
     */
    @POST
    @Path("/shopping/upgrades/by-journey")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Compute total price of selected upgrade options for this journey", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = JourneyUpgradePriceReply.class),
    ...
    public JourneyUpgradePriceReply priceUpgradesFromJourney(@QueryParam("journeyId")String journeyId, JourneyUpgradePriceRequest upgradeJourneyPriceInput);

    /**
     * Retrieve possible upgrade options for this journey
     *
     * This is a shopping query that returns possible upgrade options for a journey, as well as any relevant account balances. It will return all possible upgrade options along with their unitary price. Once certain upgrades have been selected, a POST call to /shopping/upgrades/by-journey can be performed to get the final total price 
     *
     */
    @GET
    @Path("/shopping/upgrades/by-journey")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Retrieve possible upgrade options for this journey", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "OK", response = JourneyUpgradeShopReply.class),
        @ApiResponse(code = 400, message = "Bad request", response = Messages.class),
        @ApiResponse(code = 404, message = "No journey with that id", response = Messages.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = Messages.class) })
    public JourneyUpgradeShopReply shopUpgradesFromJourney(@QueryParam("journeyId")String journeyId);
}
