...
import com.koreanair.external.dx.vo.TeaserRequest;
import com.koreanair.external.dx.vo.TeasersListReply;
...
public interface TeaserApi  {
    ...
    /**
     * Get the teaser details.
     *
     *  Returns a list of teasers with a rich set of details: third party providers information, search criteria and recommendations. The carLoyaltyNumbers parameter can be used to apply special discounts. 
     *
     */
    @POST
    @Path("/merchandising/teasers")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get the teaser details.", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "The merchandising teaser information for a given order", response = TeasersListReply.class),
        @ApiResponse(code = 400, message = "Bad request", response = WarningMessage.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = ErrorMessage.class) })
    public TeasersListReply retrieveTeaserDetails(TeaserRequest inputParams, @QueryParam("lang")String lang, @QueryParam("currencyCode")String currencyCode);

    /**
     * Get the merchandising teasers.
     *
     * Returns a list of non-air teasers computed according to the content of a cart (cartId provided in input) and based on rules defined in AAM for a given airline. For more details about AAM please see Amadeus Anytime Merchandising.          Returns a list of teaser items computed according to the AAM matching rules for the provided cart. Each teaser item is made of: - a set of teaser common attributes (teaserId, recommendationType, priorityOrder, mediaId ...) - a set search request criteria for the type of recommended product (car, hotel, insurance ...) Note: at this step no actual non-air offer is returned. 
     *
     */
    @GET
    @Path("/merchandising/teasers/by-cart")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get the merchandising teasers.", tags={  })
    @ApiResponses(value = { 
        @ApiResponse(code = 200, message = "The merchandising teaser information for a given cart", response = TeasersListReply.class),
        @ApiResponse(code = 400, message = "Bad request", response = WarningMessage.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = ErrorMessage.class) })
    public TeasersListReply retrieveTeaserInfoByCart(@QueryParam("cartId")String cartId, @QueryParam("recommendationTypes")List<String> recommendationTypes);

    /**
     * Get the merchandising teasers.
     *
     * Returns a list of non-air teasers computed according to the content of an order (orderId provided in input) and based on rules defined in AAM for a given airline. For more details about AAM please see Amadeus Anytime Merchandising. Returns a list of teaser items computed according to the AAM matching rules for the provided order. Each teaser item is made of: - a set of teaser common attributes (teaserId, recommendationType, priorityOrder, mediaId ...) - a set search request criteria for the type of recommended product (car, hotel, insurance ...) Note: at this step no actual non-air offer is returned. 
     *
     */
    @GET
    @Path("/merchandising/teasers/by-order")
    @Consumes({ "application/json" })
    @Produces({ "application/json" })
    @ApiOperation(value = "Get the merchandising teasers.", tags={  })
    @ApiResponses(value = { 
        ...
        @ApiResponse(code = 400, message = "Bad request", response = WarningMessage.class),
        @ApiResponse(code = 200, message = "Unexpected error", response = ErrorMessage.class) })
    public TeasersListReply retrieveTeaserInfoByOrder(@QueryParam("orderId")String orderId, @QueryParam("recommendationTypes")List<String> recommendationTypes);
}
